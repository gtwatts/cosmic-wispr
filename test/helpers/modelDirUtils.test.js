...
describe("modelDirUtils cache policy (#1279, #1399)", () => {
  ...
  function createRedirectedWindowsFixture() {
    ...
    const legacyRoot = path.join(home, ".cache", "cosmic-wispr");
    const redirectedProfile = path.join(tempRoot, "RedirectedUsers", "stan");
    const redirectedRoot = path.join(redirectedProfile, ".cache", "cosmic-wispr");
  ...
  beforeEach(() => {
    ...
    delete process.env.COSMIC_WISPR_CACHE_ROOT;
  ...
  it("pathHasProblematicChars flags non-ASCII and spaces", () => {
    ...
    assert.strictEqual(pathHasProblematicChars("C:\\Users\\Anton\\.cache\\cosmic-wispr"), false);
    assert.strictEqual(pathHasProblematicChars("C:\\Users\\Антон\\.cache\\cosmic-wispr"), true);
    assert.strictEqual(pathHasProblematicChars("C:\\Users\\詩涵\\.cache\\cosmic-wispr"), true);
    assert.strictEqual(pathHasProblematicChars("C:\\Users\\Stan Shih\\.cache\\cosmic-wispr"), true);
  });

  it("honors COSMIC_WISPR_CACHE_ROOT when ASCII-safe", () => {
    setPlatform("win32");
    const override = path.join(tempRoot, "ascii-cache");
    process.env.COSMIC_WISPR_CACHE_ROOT = override;
  ...
  it("gives COSMIC_WISPR_CACHE_ROOT precedence over an ASCII-safe Windows home", () => {
    setPlatform("win32");
    const home = path.join(tempRoot, "Users", "stan");
  ...
  it("honors an absolute XDG_CACHE_HOME on Linux", () => {
    ...
    assert.strictEqual(getCacheRoot(), path.join(xdgCacheHome, "cosmic-wispr"));
  });

  it("gives COSMIC_WISPR_CACHE_ROOT precedence over XDG_CACHE_HOME", () => {
    setPlatform("linux");
    const home = path.join(tempRoot, "home", "stan");
    const override = path.join(tempRoot, "custom-cache");
    process.env.COSMIC_WISPR_CACHE_ROOT = override;
  ...
  it("keeps the home cache when XDG_CACHE_HOME is relative", () => {
    ...
    assert.strictEqual(getCacheRoot(), path.join(home, ".cache", "cosmic-wispr"));
  });
  ...
  it("honors COSMIC_WISPR_CACHE_ROOT on macOS", () => {
    setPlatform("darwin");
    const home = path.join(tempRoot, "Users", "stan");
    const override = path.join(tempRoot, "custom-cache");
    process.env.COSMIC_WISPR_CACHE_ROOT = override;
  ...
  it("falls back to ProgramData when redirected USERPROFILE is unsafe", () => {
    ...
    assert.strictEqual(getCacheRoot(), path.join(programData, "CosmicWispr", "cache"));
  });
  ...
  it("skips an unsafe ProgramData fallback", () => {
    ...
    assert.strictEqual(getCacheRoot(), path.join(systemDrive, "CosmicWispr", "cache"));
  });
  ...
  it("falls back to ProgramData cache when home cache path is non-ASCII", () => {
    ...
    assert.strictEqual(root, path.join(programData, "CosmicWispr", "cache"));
  ...
  it("never overwrites models already present at the safe root", () => {
    ...
    const home = path.join(tempRoot, "使用者", "詩涵");
    const legacyDir = path.join(home, ".cache", "cosmic-wispr", "whisper-models");
    fs.mkdirSync(legacyDir, { recursive: true });
    fs.writeFileSync(path.join(legacyDir, "ggml-base.bin"), "old");

    const newDir = path.join(programData, "CosmicWispr", "cache", "whisper-models");
